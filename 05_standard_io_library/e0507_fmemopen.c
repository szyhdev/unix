...
static int mstream_seek(void *, fpos_t, int);
...
FILE *funopen(struct memstream *ms, int (read)(void *, char *, int), int (write)(void *, const char *, int),
        int (seek)(void *, fpos_t, int), int (close)(void *));
...
static int mstream_seek(void *cookie, fpos_t pos, int whence)
{
    int off;
    struct memstream *ms = cookie;
